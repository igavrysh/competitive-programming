	...
	public int mostBooked(int n, int[][] meetings) {
		...
		for (int i = 0; i < meetings.length; i++) {
            while (!Q.isEmpty() && Q.peek()[1] <= meetings[i][0]) {
                int[] freed = Q.remove();
                availableRooms.add(freed[0]);
            }
            
            if (Q.size() < n) {
                int idRoom = availableRooms.remove();
                Q.offer(new int[]{idRoom, meetings[i][1]});
                fq[idRoom]++;
                int newFq = fq[idRoom];
                ...
                int[] earliestFree = Q.remove();
                availableRooms.add(earliestFree[0]);
                int endTime = earliestFree[1] + (meetings[i][1] - meetings[i][0]);

                int idRoom = availableRooms.remove();
                Q.offer(new int[]{idRoom, endTime});
                fq[idRoom]++;
                int newFq = fq[idRoom];
                if (newFq > max_fq || (newFq == max_fq && idRoom < max_fq_room)) {
                    max_fq = fq[idRoom];
