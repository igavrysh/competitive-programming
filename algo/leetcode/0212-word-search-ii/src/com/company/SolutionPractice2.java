...
public class SolutionPractice2 {
    ...
    public List<String> findWords(char[][] board, String[] words) {
        ...
        for (int i = 0; i < rows; i++) {
            for (int j = 0; j < cols; j++) {
                BT(i, j, board, root, new StringBuilder(), output);
            }
        }
        return output;
    }
    ...
    private void BT(int i, int j, char[][] board, Node parent, StringBuilder sb, List<String> output) {
        int rows = board.length;
        int cols = board[0].length;
        if (board[i][j] == '#' ) {
            return;
        ...
        if (child.isEOW) {
            output.add(sb.append(board[i][j]).toString());
        ...
        for (int d = 0; d < dir.length; d++) {
            ...
            if (newI >= 0 && newI < rows && newJ >= 0 && newJ < cols) {
                ...
                sb.append(board[i][j]);
                board[i][j] = '#';
                BT(newI, newJ, board, child, sb, output);
                board[i][j] = tmp;
                sb.deleteCharAt(sb.length()-1);
            }
        }
