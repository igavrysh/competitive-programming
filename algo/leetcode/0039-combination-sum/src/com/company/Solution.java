...
public class Solution {
    ...
    private void combinations(List<Integer> acc, List<List<Integer>> output, int target, int[] candidates, int startIndex, Set<String> cache) {
        ...
        combinations(acc, output, nextTarget1, candidates, nextStartIndex1, cache);
        acc.remove(acc.size() - 1);

        int nextStartIndex2 = startIndex;
        int nextTarget2 = target - candidates[startIndex];
        acc.add(candidates[startIndex]);
        combinations(acc, output, nextTarget2, candidates, nextStartIndex2, cache);
        acc.remove(acc.size() - 1);

        int nextStartIndex3 = startIndex + 1;
        int nextTarget3 = target;
        combinations(acc, output, nextTarget3, candidates, nextStartIndex3, cache);
    }
}
